...
class AkshareHKDataProvider(LocalCSVDataProvider):
    """Sync HK market data with symbol-level incremental checkpoint and resume."""
    ...
    def sync_to_local_dataset(
        ...
        for symbol in target_symbols:
            ...
            for attempt in range(1, max(int(retry_times), 1) + 1):
                try:
                    with tempfile.TemporaryDirectory(prefix="hk_sync_symbol_") as tmp:
                        tmp_path = Path(tmp)
            ...
            if not succeeded:
                ...
                if not continue_on_error:
                    ...
                    raise RuntimeError(f"HK sync failed for {symbol}: {last_error}")
    ...
    def _resolve_target_symbols(cls, symbols: list[str] | None, max_symbols: int | None) -> list[str]:
        ...
        try:
            master = LegacyHKProvider._fetch_security_master(symbols=None, max_symbols=max_symbols)
    ...
    def _fallback_batch_sync(
        ...
        *,
        ...
        if window is not None and not window.should_sync:
            ...
            return output_dir
        sync_start = window.sync_start if window else start
        with tempfile.TemporaryDirectory(prefix="hk_sync_batch_") as tmp:
            tmp_path = Path(tmp)
            LegacyHKProvider.sync_to_local_dataset(
                output_dir=tmp_path,
                start=sync_start,
